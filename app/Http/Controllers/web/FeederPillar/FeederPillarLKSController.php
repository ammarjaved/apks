    ...
    public function generateByVisitDate(Fpdf $fpdf, Request $req)
    {
        ...
        foreach ($data as $row) {
            ...
            if ($row->feeder_pillar_image_1 != '')
            {
                ...
                $fpdf->Image(config('globals.APP_IMAGES_URL').$row->feeder_pillar_image_1, $fpdf->GetX(), $fpdf->GetY(), 30, 30);
            }
            $fpdf->Cell(45,6);
            // $fpdf->Ln();


            if ($row->feeder_pillar_image_2 != '' )
            {
                $fpdf->Image(config('globals.APP_IMAGES_URL').$row->feeder_pillar_image_2, $fpdf->GetX(), $fpdf->GetY(), 30, 30);
            }

            $fpdf->Cell(50,6);

            if ($row->image_name_plate != ''
            // && file_exists(public_path($row->image_name_plate))
            )
            {
                $fpdf->Image(config('globals.APP_IMAGES_URL').$row->image_name_plate, $fpdf->GetX(), $fpdf->GetY(), 30, 30);
            }
        ...
        //    $fpdf->Cell(50,7,'Pembersihan iklan Haram/Banner','LTR', 0,'C',true); //GRASS
          ...
          //  $fpdf->Cell(50, 7, '& Menutup Pintu Pencawang atau','RL', 0,'C',true); //GRASS
            ...
            if ($row->image_vandalism != ''
            //  && file_exists(public_path($row->image_vandalism))
            ) {
            ...
            if ($row->image_leaning != ''
            // && file_exists(public_path($row->image_leaning))
            ) {
            ...
            if ($row->image_rust != ''
            // && file_exists(public_path($row->image_rust))
            ) {
            ...
            if ($row->images_advertise_poster != ''
            // && file_exists(public_path($row->images_advertise_poster))
            ) {
